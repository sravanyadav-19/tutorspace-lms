...
import { Users, UserCheck, Clock, BookOpen, PlusCircle, RefreshCw } from 'lucide-react'
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setRecentUsers(users.slice(0, 5).map(u => ({
        ...
        name: u.name, meta: `${u.email} • ${u.role.name}`,
        badge: u.status,
      ...
      <div className={styles.adminDashboard}>
        <div className={styles.pageHeader}>
          ...
          <div className={styles.headerRight}>
            <Button variant="secondary" onClick={fetchDashboardData}><RefreshCw size={16} style={{ marginRight: '6px' }} /> Refresh</Button>
            <Button variant="primary" onClick={() => navigate('/admin/classes/new')}><PlusCircle size={16} style={{ marginRight: '6px' }} /> Create Class</Button>
          </div>
        </div>

        {error && <div className={styles.errorBanner} role="alert">{error}<button className={styles.retryBtn} onClick={fetchDashboardData}>Retry</button></div>}
        ...
        <div className={styles.quickActions}>
          ...
          <div className={styles.actionButtons}>
            <button className={styles.actionBtn} onClick={() => navigate('/admin/users')}><Users size={18} /><span className={styles.actionLabel}>Manage Users</span>{stats.pendingUsers > 0 && <span className={styles.actionBadge}>{stats.pendingUsers}</span>}</button>
            <button className={styles.actionBtn} onClick={() => navigate('/admin/classes')}><BookOpen size={18} /><span className={styles.actionLabel}>Manage Classes</span></button>
            <button className={styles.actionBtn} onClick={() => navigate('/admin/classes/new')}><PlusCircle size={18} /><span className={styles.actionLabel}>Create Class</span></button>
            <button className={styles.actionBtn} onClick={fetchDashboardData}><RefreshCw size={18} /><span className={styles.actionLabel}>Refresh Data</span></button>
          </div>
        </div>
