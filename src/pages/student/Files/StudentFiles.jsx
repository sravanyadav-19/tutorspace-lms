...
const StudentFiles = () => {
  ...
  // ================================
  // AGGRESSIVE SHORTCUT BLOCKER
  // ================================
  useEffect(() => {
    if (!selectedFile) return

    const blockKeys = (e) => {
      if (e.ctrlKey && ['s', 'p', 'u', 'i', 'S', 'P', 'U', 'I'].includes(e.key)) {
        e.preventDefault()
        e.stopPropagation()
        return false
      }
  ...
  const getViewUrl = (fileId) => {
    return `http://localhost:5000/api/files/view/${fileId}?token=${token}`
  }
  ...
  if (selectedFile) {
    return (
      <div
        className={styles.fullPageViewer}
        ...
        <div className={styles.viewerContent}>
          ...
          {selectedFile.mimeType === 'application/pdf' && (
            <div className={styles.pdfWrapper}>
              <iframe
                src={`${getViewUrl(selectedFile.id)}#toolbar=0&navpanes=0&scrollbar=1&statusbar=0&messages=0`}
                className={styles.pdfFrame}
                title={selectedFile.description || selectedFile.originalName}
              />
              
              {/* CLICK SHIELD: Catches right clicks but leaves scrollbar open */}
              <div
                className={styles.pdfClickShield}
                onContextMenu={(e) => { e.preventDefault(); e.stopPropagation(); return false; }}
                onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}
              />

              {/* Watermark overlay */}
              <div className={styles.pdfWatermark}>
          ...
          {selectedFile.mimeType === 'image/png' && (
            ...
            >
              ...
              >
                <img
                  src={getViewUrl(selectedFile.id)}
                  alt={selectedFile.description || selectedFile.originalName}
                  className={`
                    ${styles.viewerImage}
            ...
            <div className={styles.mainContent}>
              <div className={styles.filesSection}>
                <div className={styles.filesSectionHeader}>
                  <h2 className={styles.sectionTitle}>📁 {selectedClass?.name}</h2>
                    ...
                    {files.map(file => (
                      ...
                      >
                        ...
                        <div className={styles.fileCardInfo}>
                          <p className={styles.fileCardName}>
                            {file.description || file.originalName}
                          </p>
                          <p className={styles.fileCardMeta}>
                            {file.mimeType === 'application/pdf' ? 'PDF' : 'PNG'} • {file.formattedSize}
                          </p>
