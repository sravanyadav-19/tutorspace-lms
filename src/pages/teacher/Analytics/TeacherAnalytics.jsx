...
const TeacherAnalytics = () => {
  ...
  const promptReleaseResults = (quizId) => {
    setConfirmModal({ open: true, quizId })
  }
    ...
    <DashboardLayout userRole="teacher">
      <div className={styles.analyticsPage}>
            ...
            <div className={styles.mainContent}>
              {!selectedQuiz ? (
                <div className={styles.selectQuizPrompt}>
                  ...
                  <p className={styles.emptyText}>Choose a quiz from the left to view analytics</p>
                </div>
              ) : (
                <>
                  <div className={styles.quizAnalyticsHeader}>
                    <div>
                      ...
                      <p className={styles.quizAnalyticsMeta}>
                        {selectedClass?.name} • {selectedQuiz._count?.questions || 0} questions • {submissions.length} submissions
                      </p>
                    </div>
                    <button className={styles.releaseBtn} onClick={() => promptReleaseResults(selectedQuiz.id)}>
                      📊 Release Results
                    </button>
                  </div>
                  ...
                  <div className={styles.submissionsSection}>
                    <h3 className={styles.submissionsSectionTitle}>👥 Student Submissions</h3>
                    {submissionsLoading ? <SkeletonCard /> : submissions.length === 0 ? (
                      <div className={styles.emptySubmissions}><p>No submissions yet for this quiz.</p></div>
                    ) : (
                          ...
                          return (
                            <div key={submission.id} className={styles.tableRow}>
                              <div className={styles.studentInfo}>
                                <div className={styles.studentAvatar}>{submission.student?.name?.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)}</div>
                                <div><p className={styles.studentName}>{submission.student?.name}</p><p className={styles.studentEmail}>{submission.student?.email}</p></div>
                              </div>
                              ...
                              <span className={`${styles.releasedStatus} ${submission.isReleased ? styles.released : styles.notReleased}`}>
                                {submission.isReleased ? '✅ Released' : '⏳ Pending'}
                              </span>
                            </div>
                          )
                        })}
        ...
        <ConfirmModal
          ...
          message="Are you sure you want to release results to all students? They will be able to see their scores and answer reviews. This cannot be undone."
