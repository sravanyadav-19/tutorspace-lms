...
import { ArrowLeft, FileText, Image, BookOpen, RefreshCw, Upload, Eye, Trash2, X, ZoomIn, Target, AlertCircle, Search } from 'lucide-react'
...
const TeacherFiles = () => {
  ...
  useEffect(() => { if (selectedClass?.id) fetchFiles(selectedClass.id) }, [selectedClass])
  ...
  const handleUpload = async () => {
    ...
    if (!selectedClass?.id) { setError('Please select a class first'); return }
    ...
    try {
      ...
      setSuccess(`"${uploadedFile?.originalName || 'File'}" uploaded successfully!`)
      ...
      <div className={styles.fullPageViewer}>
        <div className={styles.viewerHeader}>
          <button className={styles.backBtn} onClick={handleClosePreview}>
            <ArrowLeft size={18} style={{ marginRight: '6px' }} /> Back to Files
          </button>
        ...
        <div className={styles.viewerContent}>
          ...
          {previewFile.mimeType === 'image/png' && (
            <div
              className={`${styles.imageContainer} ${zoomed ? styles.imageContainerZoomed : ''}`}
      ...
      <div className={styles.filesPage}>
        ...
        <div className={styles.pageHeader}>
          <div>
            <h1 className={styles.pageTitle}>
              ...
              Class Materials & Files
            </h1>
            <p className={styles.pageSubtitle}>Upload and share PDF documents and PNG study guides with students</p>
          </div>
        ...
        {loading ? <SkeletonCard /> : (classes || []).length === 0 ? (
          ...
          <div className={styles.contentLayout}>
            ...
            {/* Main Upload & File Management Content */}
            <div className={styles.mainContent}>
              <div className={styles.uploadSection}>
                ...
                <div className={styles.fileTypeNotice}>
                  <FileText size={14} color="#c64545" /> PDF <span>and</span> <Image size={14} color="#5db8a6" /> PNG <span>files allowed • Max 10MB</span>
                </div>
                  ...
                  {selectedFile ? (
                    <div className={styles.selectedFile}>
                      <div className={styles.selectedFileIcon}>
                        {selectedFile.type === 'application/pdf' ? <FileText size={28} color="#c64545" /> : <Image size={28} color="#5db8a6" />}
                      </div>
                      <div className={styles.selectedFileInfo}>
                        <p className={styles.selectedFileName}>{selectedFile.name}</p>
                        <p className={styles.selectedFileSize}>{getFileSize(selectedFile)} • {selectedFile.type === 'application/pdf' ? 'PDF Document' : 'PNG Image'}</p>
                      </div>
                      <button
                    ...
                    <div className={styles.dropZoneContent}>
                      <div className={styles.dropZoneIcons}>
                        <FileText size={32} color="#c64545" />
                        <Image size={32} color="#5db8a6" />
                      </div>
                      <p className={styles.dropZoneText}>Drag & drop a PDF or PNG document here, or <span className={styles.browseLink}>browse file</span></p>
                    </div>
                  )}
                </div>

                <div className={styles.descriptionField}>
                  <label className={styles.descriptionLabel}>File Title / Description</label>
                  <input
                    type="text"
                    className={styles.descriptionInput}
                    placeholder="e.g. Lecture 5 Slides, Homework 2 Study Guide..."
                    value={description}
                ...
                >
                  {uploading ? 'Uploading File...' : <><Upload size={16} style={{ marginRight: '6px' }} /> Upload File</>}
                </Button>
              </div>
              ...
              <div className={styles.filesSection}>
                <div className={styles.sectionHeader}>
                  <h2 className={styles.sectionTitle}>
                    Uploaded Files <span className={styles.fileCount}>{(files || []).length} files</span>
                  </h2>
                ...
                {filesLoading ? <SkeletonGrid count={3} type="card" /> : filteredFiles.length === 0 ? (
                  <EmptyState
                    icon="files"
                    title={fileSearch ? 'No Files Found' : 'No Files Uploaded Yet'}
                    message={fileSearch ? 'Try a different search query' : `No study materials have been uploaded to ${selectedClass?.name} yet.`}
                    size="sm"
                  />
                ) : (
                    ...
                    {filteredFiles.map(file => (
                      <div key={file.id} className={styles.fileCard}>
                        <div className={styles.fileIcon}>
                          {file.mimeType === 'application/pdf' ? <FileText size={36} color="#c64545" /> : <Image size={36} color="#5db8a6" />}
                        </div>
                        <div className={styles.fileInfo}>
                          <p className={styles.fileName}>{file.description || file.originalName}</p>
                        ...
                        <div className={styles.fileActions}>
                          ...
                          <button
                            ...
                            onClick={() => promptDelete(file.id, file.originalName)}
                            title="Delete file"
        ...
        <ConfirmModal
          ...
          title="Delete File"
          message={`Delete "${confirmModal.fileName}"? Students will immediately lose access to this material.`}
